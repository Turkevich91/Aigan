...
The public media acquisition boundary is registered as `media_acquisition` and is disabled by default. When `MEDIA_ACQUISITION_ENABLED=true`, explicit private, reply-to-bot, or group-invocation requests about public media URLs can use a metadata-only `yt-dlp` probe through the `media_context` route. V1 returns a grounded YouTube transcript answer when the existing transcript tool can help, or an honest metadata-only/unavailable response for TikTok/Instagram-style links; it does not promise TikTok/Instagram reliability, add new video-understanding providers, or enable passive group handling. It records only stable platform/backend/capability fields and sanitized failure categories such as `unsupported_url`, `auth_or_rate_limited`, `challenge_required`, `duration_limit`, or `metadata_failed`; it must not log raw media URLs, provider stderr, source text, transcripts, frame OCR, usernames, tokens, or local paths.
