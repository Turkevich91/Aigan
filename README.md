...
The public media acquisition boundary is registered as `media_acquisition` and is disabled by default. When `MEDIA_ACQUISITION_ENABLED=true`, private bare supported media URLs, explicit reply-to-bot requests, and group-invocation requests about supported public media URLs can use a `yt-dlp` metadata probe through the `media_context` route. Follow-up prompts such as "what is this reel?" may resolve the URL from a replied/quoted message, Telegram link preview metadata, or recent passive context, while ordinary group links still remain silent/passive. Current, replied, quoted, and link-preview URLs take priority over recent memory; recent fallback scans newest-first and ignores bot/generated summaries as URL anchors. When bounded acquisition is safe, the route may also download a temporary capped media file, pass it through `media_frames`, summarize selected frames through the vision path, and save the generated summary as source context only. YouTube transcript/caption context remains preferred when available; TikTok/Instagram-style links remain best-effort and may degrade to metadata-only or unavailable. Portrait/vertical media is requested with width-bounded format alternatives before unbounded video fallbacks, while final byte caps still enforce safety. This does not add new video-understanding providers or passive group handling. It records only stable platform/backend/capability fields and sanitized failure categories such as `unsupported_url`, `auth_or_rate_limited`, `challenge_required`, `duration_limit`, `file_too_large`, `download_failed`, or `visual_summary_failed`; it must not log raw media URLs, provider stderr, source text, transcripts, frame OCR, usernames, tokens, or local paths.
